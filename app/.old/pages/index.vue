...
class StateManager {
  ...
  constructor(stage?: State) {
    this.state = stage ? stage : State.START;
  }
  ...
  next() {
    if (this.state < Object.keys(State).length) this.state++;
  }
  ...
  isSummary(): boolean {
    return this.state === State.SUMMARY;
  }
...
const stage = ref<StateManager>(new StateManager());
    ...
    >
      <slot v-if="stage.isStart()">
        ...
        <p class="text-justify">
          Turniej odbędzie się 25 Maja 2024 na Kompleksie Boisk "Orlik" przy ulicy Ułanów 8 w Knurowie. Aby zapisać
          swoją drużynę na turniej koszykówki ulicznej Knurowski Streetball Kapitan musi wypełnienić formularz, a
          następnie potwierdzić go drogą e-mail.
          <span>
